...
async function clearData() {
    ...
    console.log("Menghapus log rekonsiliasi lama agar sinkron...")
    const { error: err3 } = await supabase.from('log_rekonsiliasi').delete().neq('id', 0)
    if (err3) {
        console.error("Gagal menghapus log:", err3)
    } else {
        console.log("Berhasil membersihkan data Log Rekonsiliasi.")
    }
}
